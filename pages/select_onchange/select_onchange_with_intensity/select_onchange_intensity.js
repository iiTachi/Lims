...
Page({
    ...
    onLoad: function(options) {
      ...
      app.functions.authRequest('/app/smell/training/start', 'POST', postData, function (res) {
        ...
        var postSmellStrength = that.data.post_smell_strength
  ...
  preExercise: function () {
    ...
    if (exerciseNum == 1) {
      ...
      var strengthFirstValue = null
      console.log('semll text ', postSmellTypeText)
      if (postSmellOtherType[exerciseNum - 2] != "init") {
        typeFirstValue = postSmellOtherType[exerciseNum - 2]
        subTypeFirstValue = " "
        strengthFirstValue = postSmellStrength[exerciseNum - 2]
      }
      else {
      ...
      this.setData({
        ...
        strength_firstvalue: strengthFirstValue
      })
    }
  },
  ...
  nextExercise: function () {
    ...
    var currentStrength = postSmellStrength[exerciseNum - 1]
    console.log('next ', currentOtherType)

    if (currentType == -1 && currentSubType == -1 && currentOtherType == "init" && currentStrength == -1) {
      wx.showToast({
        title: '您还没有做出选择',
        icon: 'none'
      })
      ...
      if (exerciseNum == counterI) {
          ...
          success: function (res) {
            if (res.confirm) {
              ...
              for (var i = 0; i < counterI; i++) {
                ...
                answer[i].strength = that.data.post_smell_strength[i]
              }
              // for (var i = 0; i < 7; i++) {
              ...
              for (var i = 0; i < counterI; i++) {
                ...
                if (that.data.post_smell_othertype[i] != "init") {
                  ...
                  transmissionData[i].your_answer = that.data.post_smell_type_text[i] + '->' + that.data.post_smell_subtype_text[i] + ':' + that.data.post_smell_strength[i]
                }
                transmissionData[i].correct_answer = that.data.exercise[i].type.title + '->' + that.data.exercise[i].subtype.title + ':' + that.data.post_smell_strength[i]
        ...
        if (postSmellOtherType[exerciseNum] != "init") {
          ...
          subTypeFirstValue = " "
          strengthFirstValue = postSmellStrength[exerciseNum]
        }
        else {
        ...
        this.setData({
          ...
          strength_firstvalue: strengthFirstValue == -1 ? null : strengthFirstValue
        })
      }
    }
  },
